...
public class Upload_RecoderDemo {
	...
	public static void main(String[] args) {
		...
		try{
			...
			/*  记录视频上传参数的集合，用于存储断点续传所需的相关参数 */
			Map<String, Object> recoderMap = new HashMap<String, Object>();	
			
			/* 记录视频是否开始上传，未开始上传，则设置为false，否则true*/
			recoderMap.put("start_flag", false);
			
			/* 记录视频是否上传结束，未结束，则设置为false，否则true */
			recoderMap.put("end_flag", false);

						
			QueryVideoIDorWatermarkIDParam queryVideoIDParam = null;
			
			/* 当是否未上传结束并且上传过程中未出现错误，则可以续传视频*/
			while(!(Boolean) recoderMap.get("end_flag") && (recoderMap.get("error") == null)){
				 queryVideoIDParam  = vclient.uploadVideoWithRecorder(filePath, initParamMap, recoderMap);
			}
